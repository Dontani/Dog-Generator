...
.then(data =>{
  ...
  const option = document.createElement('option')//adding options to the drop down menu, loop through array using 'for' loop..basically <option></option>
  option.value = breedsArray[i]//<option value = 'breed'>
    option.innerText = breedsArray[i]//puts breed inside the text of the option element 
    select.appendChild(option)//appends each option to the drop down menu..see select var above
  }
})
...
const getDogImg = url => {
  ...
  .then(data =>{
    img.src = data.message//extract message from json and attatch to img tag as new src
  })
}
const dogInfo = {
...
assignsex(){
  ...
  assignName(array){
    ...
    document.getElementById('sex').innerHTML = `S: ${this.sex}`
  },

assignAge(){
  this.age = Math.floor(Math.random() * 16+1)//returns number from 1-16 because m.random max is 1 so *16 sets the range..and 1 sets the min value
  ...
  assignLikes (){
    this.likes = this.yatesShuffle(this.likesList).slice(0,2)//slice returns first two elements
